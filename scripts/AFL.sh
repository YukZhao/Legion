...
cleanup_run_dir() {
    ...
    if [[ -e "$dir" ]]; then
        echo "[LEGION]warning: failed to fully remove $dir before AFL run"
        return 1
    fi
    return 0
}
...
build() {
    ...
    if use_noasan_fuzz_build; then
        echo "[LEGION]building AFL fuzzing target with AutoFZ justafl flags"
        ...
        echo "[LEGION]building AFL fuzzing target with AutoFZ aflasan flags"
	...
	popd || exit
	popd || exit
	cp -p code_afl/"$SRC"/app build/aflapp
    if use_noasan_fuzz_build; then
        cp -p build/aflapp build/aflapp_noasan
    else
        cp -p build/aflapp build/aflapp_asan
    fi

}

run(){
    #sudo $AFL_BIN/afl-system-config
    ...
    flush_output() {
        ...
        copy_queue_files "$outfolder" run_afl/output/fuzzer*/queue
    }
    ...
    cleanup_run_dir run_afl || return 1
    mkdir -p run_afl/input run_afl/output
    #get initial seeds, if any, to in
    copy_seed_dir "${LEGION_INITIAL_DIR:-initial}" run_afl/input
    if ! has_regular_files run_afl/input; then
    ...
    # screen -dmS afl-main timeout --foreground "$time" $AFL_BIN/afl-fuzz -i input -o output -M fuzzer0 $dict -- ../build/aflapp
    timeout --foreground "$time" $AFL_BIN/afl-fuzz -m none -t "$LEGION_AFL_TIMEOUT" -i input -o output -M fuzzer0 $dict -- ../build/aflapp &
    pids+=("$!")
    ...
    while [ "$i" -lt "$threads" ]
        do 
        #screen -dmS afl-"$i" timeout --foreground "$time" $AFL_BIN/afl-fuzz -i input -o output -S fuzzer$i $dict -- ../build/aflapp
        timeout --foreground "$time" $AFL_BIN/afl-fuzz -m none -t "$LEGION_AFL_TIMEOUT" -i input -o output -S fuzzer$i $dict -- ../build/aflapp &
        pids+=("$!")
    i=$((i + 1))
    done
    ...
    echo "[LEGION]$threads instances of AFL run finished in $time seconds"

    #find run_afl/output/fuzzer0/queue -type f -name '*' -print0 | xargs -0 cp --target-directory="$outfolder"
...
append() {
    ...
    while [[ ! -d run_afl/output && "$waited" -lt "$wait_limit" ]]; do
        sleep 1
        waited=$((waited + 1))
    done

    if [[ ! -d run_afl/output ]]; then
    ...
    trap handle_term TERM INT
    pushd run_afl >/dev/null || exit
    copy_seed_dir "${LEGION_INITIAL_DIR:-../initial}" input
    if [[ -n "${LEGION_APPEND_WORKER_INDEX:-}" ]]; then
        next_index="$LEGION_APPEND_WORKER_INDEX"
    else
        next_index=$(next_afl_worker_index output)
    fi

    while [[ "$i" -lt "$threads" ]]; do
        timeout --foreground "$time" "$AFL_BIN/afl-fuzz" -m none -t "$LEGION_AFL_TIMEOUT" -i input -o output -S "fuzzer$next_index" $dict -- ../build/aflapp &
        pids+=("$!")
...
update_seed() {
    source=$1
    sync_seed_dir_into_afl_peer_queue "$source" run_afl/output
}
...
minimize() {
    ...
    in_dir=$1
    out_dir=$2
    #command="$AFL_BIN/afl-cmin -m none -i $in_dir -o $out_dir -- ./build/aflapp"
    #echo $command
    time $AFL_BIN/afl-cmin -m none -i "$in_dir" -o "$out_dir" -- ./build/aflapp
...
usage(){
    ...
    echo "[LEGION]USAGE: AFL.sh build -zip|-gz <file> or AFL.sh run|append <time> <threads> <output folder> [-d]"
}
