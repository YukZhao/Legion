...
export AFLFAST_BIN="${AFLFAST_BIN:-$TOOLS_ROOT/AFLFast}"
...
cleanup_run_dir() {
    ...
    if [[ -e "$dir" ]]; then
        echo "[LEGION]warning: failed to fully remove $dir before AFLFast run"
        return 1
    fi
    return 0
}
...
build() {
    ...
    rm -rf code_aflfast/"$SRC"
    mkdir code_aflfast
    cp "$2" code_aflfast/"$ARCHIVE"
    pushd code_aflfast || exit
    ...
    if use_noasan_fuzz_build; then
        echo "[LEGION]building AFLFast fuzzing target with AutoFZ justafl flags"
        ...
        echo "[LEGION]building AFLFast fuzzing target with AutoFZ aflasan flags"
	...
	popd || exit
	popd || exit
	cp -p code_aflfast/"$SRC"/app build/aflfastapp
    if use_noasan_fuzz_build; then
        cp -p build/aflfastapp build/aflfastapp_noasan
    else
        cp -p build/aflfastapp build/aflfastapp_asan
    fi

}

run(){
    #sudo $AFLFAST_BIN/afl-system-config
    ...
    flush_output() {
        ...
        copy_queue_files "$outfolder" run_aflfast/output/fuzzer*/queue
    }

    handle_term() {
        terminate_requested=1
        echo "[LEGION]AFLFast received termination request, flushing current queue snapshot"
    }

    trap handle_term TERM INT
    cleanup_run_dir run_aflfast || return 1
    mkdir -p run_aflfast/input run_aflfast/output
    #get initial seeds, if any, to in
    copy_seed_dir "${LEGION_INITIAL_DIR:-initial}" run_aflfast/input
    if ! has_regular_files run_aflfast/input; then
        echo "[LEGION]no initial seeds available for AFLFast, skip this round"
        rm -rf "$outfolder"
        mkdir -p "$outfolder"
        return 0
    fi
    pushd run_aflfast || exit
    ...
    # screen -dmS afl-main timeout --foreground "$time" $AFLFAST_BIN/afl-fuzz -i input -o output -M fuzzer0 $dict -- ../build/aflfastapp
    timeout --foreground "$time" $AFLFAST_BIN/afl-fuzz -m none -t "$LEGION_AFL_TIMEOUT" -i input -o output -M fuzzer0 $dict -- ../build/aflfastapp &
    pids+=("$!")
    ...
    while [ "$i" -lt "$threads" ]
           do 
        #screen -dmS afl-"$i" timeout --foreground "$time" $AFLFAST_BIN/afl-fuzz -i input -o output -S fuzzer$i $dict -- ../build/aflfastapp
        timeout --foreground "$time" $AFLFAST_BIN/afl-fuzz -m none -t "$LEGION_AFL_TIMEOUT" -i input -o output -S fuzzer$i $dict -- ../build/aflfastapp &
        pids+=("$!")
    i=$((i + 1))
    done
    ...
    while [ "$i" -lt "$time" ] 
        ...
        echo "[LEGION-fuzzer]running AFLFast background, $i/$time seconds passed"
        sleep 60
        i=$((i + 60))
        counter=$((counter + 60))
    ...
    echo "[LEGION]$threads instances of AFLFast run finished in $time seconds"

    #find run_aflfast/output/fuzzer0/queue -type f -name '*' -print0 | xargs -0 cp --target-directory="$outfolder"
...
append() {
    ...
    while [[ ! -d run_aflfast/output && "$waited" -lt "$wait_limit" ]]; do
        sleep 1
        waited=$((waited + 1))
    done

    if [[ ! -d run_aflfast/output ]]; then
        echo "[LEGION]AFLFast append requested without an active run directory, fallback to fresh run"
        run "$@"
        return 0
    fi
    ...
    handle_term() {
        terminate_requested=1
        echo "[LEGION]AFLFast append workers received termination request"
    }

    trap handle_term TERM INT
    pushd run_aflfast >/dev/null || exit
    copy_seed_dir "${LEGION_INITIAL_DIR:-../initial}" input
    if [[ -n "${LEGION_APPEND_WORKER_INDEX:-}" ]]; then
        next_index="$LEGION_APPEND_WORKER_INDEX"
    else
        next_index=$(next_aflfast_worker_index output)
    fi

    while [[ "$i" -lt "$threads" ]]; do
        timeout --foreground "$time" "$AFLFAST_BIN/afl-fuzz" -m none -t "$LEGION_AFL_TIMEOUT" -i input -o output -S "fuzzer$next_index" $dict -- ../build/aflfastapp &
        pids+=("$!")
    ...
    popd >/dev/null || exit
    echo "[LEGION]AFLFast append finished after adding $threads slave workers"
}

update_seed() {
    source=$1
    sync_seed_dir_into_afl_peer_queue "$source" run_aflfast/output
}

# we use aflfast for potential seed minimization
minimize() {
    # this is used since the minimization seem to crash or timeout when evaluating real tools
    export AFL_SKIP_BIN_CHECK=1
    in_dir=$1
    out_dir=$2
    #command="$AFLFAST_BIN/afl-cmin -i $in_dir -o $out_dir -- ./build/aflfastapp"
    #echo $command
    time $AFLFAST_BIN/afl-cmin -i "$in_dir" -o "$out_dir" -- ./build/aflfastapp
...
usage(){
    ...
    echo "[LEGION]USAGE: AFLFast.sh build -zip|-gz <file> or AFLFast.sh run|append <time> <threads> <output folder> [-d]"
}
...
if [[ "$1" == "build" ]]; then
    echo "[LEGION]AFLFast build"
    build "${@:2}"
elif [[ "$1" == "run" ]]; then
    echo "[LEGION]AFLFast running"
    run "${@:2}"
elif [[ "$1" == "append" ]]; then
    echo "[LEGION]AFLFast appending workers"
    append "${@:2}"
elif [[ "$1" == "update" ]]; then
    echo "[LEGION]AFLFast updating seeds"
    update_seed "${@:2}"
elif [[ "$1" == "minimize" ]]; then
    echo "[LEGION]AFLFast minimizing seeds"
    minimize "${@:2}"
else
    usage
    exit 1
